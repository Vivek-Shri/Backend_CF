...
_env_candidates = [
    ...
    os.path.join(_script_dir, ".env.outreach"),
...
# CAMPAIGN_STEPS is a JSON array passed by Back.py via environment.
# Each step has: id, aiInstruction, type ("immediate"|"normal"), delayValue, delayUnit, enabled
CAMPAIGN_STEPS = []
_CURRENT_STEP_INDEX = 0
try:
    _raw_steps_env = os.environ.get("CAMPAIGN_STEPS", "")
    if _raw_steps_env:
        _parsed_steps = json.loads(_raw_steps_env)
        CAMPAIGN_STEPS = [s for s in _parsed_steps if isinstance(s, dict) and s.get("enabled") is not False]
...
FORM_FILL_MAX_OUTPUT_TOKENS = max(120, min(1500, _env_int("FORM_FILL_MAX_OUTPUT_TOKENS", 800)))
FORM_FILL_MAX_OUTPUT_TOKENS_RECOVERY = max(
    FORM_FILL_MAX_OUTPUT_TOKENS,
    min(1500, _env_int("FORM_FILL_MAX_OUTPUT_TOKENS_RECOVERY", FORM_FILL_MAX_OUTPUT_TOKENS + 200)),
)
...
async def safe_append_row(sheet, row: list, max_retries=6):
    # Disabled external sheet/CSV writing. All results are streamed natively via [RESULT] logs
    # to the Back.py server and stored into the PostgreSQL database.
    return True
...
async def _has_form_signal_for_discovery(page) -> bool:
    ...
    try:
        for frame in page.frames:
            ...
            if fc >= 2 or any(p in frame_url.lower() for p in KNOWN_FORM_PROVIDERS):
                return True
    except Exception:
        pass
    ...
    if not js_checked:
        try:
            return (await _count_form_fields(page)) >= 2
        except Exception:
            return False
...
KNOWN_FORM_PROVIDERS = [
    ...
    "123formbuilder.com", "gravity", "wpforms", "planiteasy.com",
    "honeybook.com", "dubb.com", "vouchedfor.co.uk", "calendly.com",
]
...
def _build_field_catalog_json(elements, limit: int = 30) -> str:
    ...
    compact = []
    for _, el in ranked[: max(1, int(limit or 1))]:
        entry = _compact_field_for_prompt(el)
        # Strip zero-value/empty keys to save tokens
        entry = {k: v for k, v in entry.items() if v not in ("", None, False, 0, [])}
...
async def get_confirmation(page, target=None, original_url="", company_name="", worker_index=-1,
    ...
    print(f"   {tag} [Confirm] no signal -> No (no confirmation detected)")
    return "No", "Submission not confirmed (no success message, redirect, or explicit confirmation found)"
...
async def process_form(pw, company_name, url, sheet, lead_index, total,
    ...
    try:
        ...
        if not submitted:
            ...
            bw_kb = round(bw["bytes"] / 1024, 1)
            await safe_append_row(sheet, _build_row(
                company_name, url, "No", submit_fail_reason,
                captcha_status, proxy_label, str(bw_kb),
                _lead_tokens(),
                filled_fields=form_data, message_sent=pitch, subject_text=subject,
                time_taken=_lead_time_taken(),
...
def load_leads(csv_path=None):
    # Check if the argument is a file (JSON or CSV) BEFORE the single-domain check,
    # because _looks_like_domain_or_url_arg matches ".json" / ".csv" as domain-like strings.
    if csv_path and (csv_path.endswith(".json") or csv_path.endswith(".csv") or os.path.isfile(csv_path)):
        pass  # Fall through to the file-loading logic below
    elif csv_path and _looks_like_domain_or_url_arg(csv_path):
    ...
    if csv_path:
        try:
            ...
            if csv_path.endswith(".json"):
                ...
                print(f"[Input] Loaded {len(leads)} leads from JSON: {csv_path}")
                return leads
...
async def main():
    ...
    print(f"  Campaign steps    : {len(CAMPAIGN_STEPS)} enabled step(s)")
    for si, step in enumerate(CAMPAIGN_STEPS):
        stype = step.get("type", "immediate")
        delay = f" (delay: {step.get('delayValue', 0)} {step.get('delayUnit', 'days')})" if stype == "normal" else ""
        instr_preview = str(step.get("aiInstruction", ""))[:60]
        print(f"    Step {si+1}: [{stype}]{delay} {instr_preview}{'...' if len(str(step.get('aiInstruction', ''))) > 60 else ''}")
    ...
    if csv_path and csv_path.endswith(".csv"):
        abort_path = csv_path.replace(".csv", ".abort")
    
    async def _abort_checker():
        if not abort_path: return
        while not _STOP_FLAG.is_set():
            if os.path.exists(abort_path):
                print(f"\n[Abort] Found abort file: {abort_path}. Stopping run gracefully.")
                _STOP_FLAG.set()
                try:
    ...
    if abort_path:
        asyncio.create_task(_abort_checker())
    ...
    asyncio.to_thread(generate_ai_pitch_and_subject, next_company, worker_index),
                ...
                steps_to_run = CAMPAIGN_STEPS if CAMPAIGN_STEPS else [{"aiInstruction": os.environ.get("AI_INSTRUCTION", ""), "type": "immediate"}]
                
                # Execute consecutive 'immediate' steps. (Delayed 'normal' steps require cross-session scheduling via DB)
                for step_idx, step in enumerate(steps_to_run):
                    step_type = step.get("type", "immediate")
                    delay_val = int(step.get("delayValue", 0) or 0)
                    
                    if step_idx > 0 and (step_type == "normal" and delay_val > 0):
                        # Stop processing further steps for this contact; DB scheduler will pick it up later
                        break
                        
                    _CURRENT_STEP_INDEX = step_idx
                    os.environ["AI_INSTRUCTION"] = str(step.get("aiInstruction", ""))
                    
                    print(f"\n[Worker#{worker_index}] [{lead_index}/{total}] {company[:35]} | Step {step_idx+1} | proxy-slot={worker_index % len(PROXY_LIST)}")
